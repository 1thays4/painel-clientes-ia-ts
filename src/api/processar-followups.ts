...
import { verificarClientesParaFollowup, marcarClienteParaFollowup } from '../utils';
...
export async function processarFollowups(req: Request, res: Response) {
  try {
    ...
    const { success, data: clientesParaFollowup, error } = await verificarClientesParaFollowup();
    
    if (!success || error) {
      console.error('Erro ao verificar clientes para follow-up:', error);
      return res.status(500).json({ error: 'Erro ao verificar clientes para follow-up' });
    }
    ...
    console.log(`Encontrados ${clientesParaFollowup.length} clientes para follow-up`);
    
    // Chamar o webhook do n8n para processar os follow-ups
    try {
      await axios.post(config.N8N_FOLLOWUP_WEBHOOK_URL || 'http://localhost:5678/webhook/followup', {
