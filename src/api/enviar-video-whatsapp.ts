...
export async function enviarVideoWhatsApp(req: Request, res: Response) {
  try {
    ...
    if (!whatsappNumero || !videoUrl) {
      return res.status(400).json({ error: 'Dados incompletos. Número WhatsApp e URL do vídeo são obrigatórios.' });
    }
    ...
    return res.status(500).json({ 
      ...
      message: "Desculpe, ocorreu um erro ao processar sua solicitação de vídeo."
    });
  }
}