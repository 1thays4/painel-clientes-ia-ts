...
export async function buscarClientePorToken(
  token: string,
  ...
  try {
    ...
    if (!cliente.mensagens_limite) {
      ...
      const planoConfig =
        config.planos[cliente.plano as keyof typeof config.planos];
...
export async function buscarHistoricoMensagens(
  ...
  try {
    console.log(
      "Buscando histórico para cliente ID:",
      clienteId,
      "limite:",
      limite,
      "offset:",
      ...
      return [
        {
          ...
          resposta:
            'Para usar a IA no WhatsApp, basta enviar uma mensagem para o número cadastrado com o prefixo "IA:".',
        ...
        },
        {
          ...
          resposta:
            "No plano básico, você tem direito a 100 mensagens por mês. Para mais mensagens, considere fazer upgrade para um plano superior.",
    ...
    if (data && data.length > 0) {
      ...
      const clienteIds = Array.from(
        new Set(
          data
            .map((msg: { cliente_id: any }) => msg.cliente_id)
            .filter(Boolean),
        ),
      );
      ...
      const clienteFinaisIds = Array.from(
        new Set(
          data
      ...
      const clientesFinaisDoCliente = clientesFinaisData.filter(
        (clienteFinal: { cliente_id?: string | number }) =>
          clienteFinal.cliente_id == clienteId,
      );
      console.log(
        `Clientes finais do cliente ${clienteId}:`,
        clientesFinaisDoCliente,
      );
      ...
      const mensagensFormatadas = data.map(
        (msg: {
          cliente_id: any;
          ...
          const clienteFinal = msg.cliente_final_id
            ? clientesFinaisMap[String(msg.cliente_final_id)] || {}
            : {};
          ...
          const whatsappCliente = formatarNumeroWhatsApp(
            (cliente as { whatsapp?: string }).whatsapp,
          );
          const whatsappClienteFinal = formatarNumeroWhatsApp(
            (clienteFinal as { whatsapp?: string })?.whatsapp,
          );

          // Usar os números de remetente e destino se disponíveis
          const numeroRemetente =
            (msg as any).numero_remetente || whatsappCliente;
          const numeroDestino =
            (msg as any).numero_destino || whatsappClienteFinal;

          // Se o número de destino for igual ao número do cliente, usar o número de remetente como destino
          const numeroDestinoFinal =
            numeroDestino === whatsappCliente ? numeroRemetente : numeroDestino;

          return {
            ...msg,
            nome_cliente:
              (cliente as { nome?: string }).nome || "Cliente sem nome",
            whatsapp_cliente: whatsappCliente,
            nome_cliente_final:
              (clienteFinal as { nome?: string })?.nome || "Usuário final",
...
(async function inicializarTokens() {
  ...
  try {
    ...
    if (atualizados > 0) {
      console.log(
        `Inicialização: ${atualizados} clientes receberam novos tokens`,
      );
    }
  } catch (error) {
...
export async function buscarClientesFinais(): Promise<any[]> {
  try {
    ...
    const { data, error } = await supabase.from("clientes_finais").select("*");
    ...
    if (error) {
      ...
      if (altError) {
        ...
        if (altError.code === "42P01") {
          // Tabela não existe
          console.log(
            "Tabela não existe. Verifique o nome correto da tabela no Supabase.",
          );
        }
      ...
      console.log(
        "Clientes finais encontrados (tabela singular):",
        altData?.length || 0,
      );
      return altData || [];
    }
...
export function processarClientesFinais(clientesFinaisData: any[]): {
  [key: string]: {
    id: string | number;
    nome?: string;
    whatsapp?: string;
...
export async function contarTotalMensagens(
  clienteId: string | number | null,
): Promise<number> {
  // Configurar o token do cliente nos cabeçalhos
  setupAuthHeaders();
...
export async function criarCliente(userData: {
  nome: string;
  plano?: string;
  whatsapp?: string;
}): Promise<Cliente | null> {
