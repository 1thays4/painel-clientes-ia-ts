...
export default async function handler(
  ...
  try {
    ...
    if (
      !config.IA_API_KEY ||
      config.IA_API_KEY === "your-openai-api-key-here"
    ) {
      console.error("❌ DEBUG: Configuração da API de IA incompleta");
      return res
        .status(500)
        .json({ error: "Configuração do sistema incompleta" });
    }
    ...
    if (!cliente) {
      console.error(
        "❌ DEBUG: Cliente não encontrado para o número:",
        numeroLimpo,
      );
      return res.status(404).json({ error: "Cliente não encontrado" });
    }
    ...
    const numeroRemetenteFormatado = formatarNumero(
      numero_remetente || numeroRemetente || numeroLimpo,
    );
    const numeroDestinoFormatado = formatarNumero(
    ...
    console.log(
      "🔍 DEBUG: Timestamp ajustado para fuso BR:",
      timestampAjustado,
    );
    ...
    if (updateError) {
      console.error(
        "❌ DEBUG: Erro ao atualizar contador de mensagens:",
        updateError,
      );
    } else {
