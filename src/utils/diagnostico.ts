...
export async function verificarTabela(nomeTabela: string): Promise<void> {
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '42P01') {
        console.log(`DIAGNÓSTICO: A tabela "${nomeTabela}" não existe no banco de dados.`);
      } else if (error.code?.startsWith('PGRST')) {
        console.log(`DIAGNÓSTICO: Problema de permissão do PostgREST. Verifique as políticas de segurança.`);
      } else {
        console.log(`DIAGNÓSTICO: Erro desconhecido. Verifique o token de API e permissões.`);
      }
    } else {
...
export async function verificarToken(): Promise<void> {
  ...
  try {
    ...
    if (error) {
      ...
      console.log('Algumas tabelas podem não estar acessíveis sem autenticação.');
    }
  } catch (error) {
    console.error('Erro ao verificar token:', error);
  }
