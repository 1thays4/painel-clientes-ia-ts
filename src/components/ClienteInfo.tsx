...
export default function ClienteInfo({ mensagem }: ClienteInfoProps) {
  ...
  const getNumeroClienteFinal = (): string => {
    ...
    if (mensagem.numero_destino && mensagem.numero_destino !== mensagem.whatsapp_cliente) {
      return mensagem.numero_destino;
    }
    
    // Prioridade 2: numero_remetente se não for igual ao whatsapp_cliente
    if (mensagem.numero_remetente && mensagem.numero_remetente !== mensagem.whatsapp_cliente) {
  ...
  return (
    <div className="mb-3 pb-2 border-b border-gray-200">
      ...
      {mensagem.nome_cliente && (
        <div className="mb-2">
          <h4 className="font-medium text-blue-600">Empresa: {mensagem.nome_cliente}</h4>
        </div>
      )}
