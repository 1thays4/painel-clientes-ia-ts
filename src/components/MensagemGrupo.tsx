...
export default function MensagemGrupo({
  ...
  const [clientesFinaisInfo, setClientesFinaisInfo] = useState<
    Record<
      string | number,
      { id: string | number; nome: string; modo: boolean }
    >
  >({});
  ...
  useEffect(() => {
    ...
    const buscarClientesFinais = async () => {
      try {
        ...
        > = {};
        data?.forEach(
          (cliente: { id: string | number; nome: any; modo: string }) => {
            // Usar o valor exato do banco de dados, sem valor padrão
            const modoEfetivo =
              cliente.modo === "true" ||
              cliente.modo === "encerrado" ||
              cliente.modo === "em aberto";
            ...
            console.log(
              `Cliente final ${cliente.id}: modo original = ${cliente.modo}, modo efetivo = ${modoEfetivo}`,
            );
          },
        );
        ...
        {msgs[0] && (
          <Paper
            ...
            sx={{
              ...
              bgcolor:
                mensagemSelecionada &&
                msgs.some((m) => m.id === mensagemSelecionada)
              ...
              ...(msgs.some((m) => novasMensagens[m.id])
          ...
          >
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
              ...
              >
                ...
                {msgs[0].cliente_final_id &&
                  clientesFinaisInfo[msgs[0].cliente_final_id] && (
                    <Box
