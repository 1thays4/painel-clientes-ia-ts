...
import {
  ...
  atualizarModoBotTodosClientesFinais,
} from "../services/cliente-final";
import {
  Card,
  CardContent,
  Typography,
...
export default function StatusModoBotCliente({
  clienteId,
  modoBotAtivo,
}: StatusModoBotClienteProps) {
  ...
  const toggleTodosModoBot = async (ativar: boolean) => {
    ...
    try {
      ...
      const sucesso = await atualizarModoBotTodosClientesFinais(
        clienteId,
        ativar,
      );
      ...
      toast.success(
        `Modo de resposta automática ${ativar ? "ativado" : "desativado"} para todos os contatos`,
      );
    } catch (error) {
      console.error("Erro:", error);
          ...
          <Box>
            ...
            <Alert severity="info" sx={{ mt: 1 }}>
              <Typography variant="body2">
                <strong>Como funciona:</strong> Quando o modo bot está
                desativado para um contato, as mensagens recebidas desse contato
                não serão respondidas automaticamente pela IA. Você poderá
                responder manualmente a essas mensagens através do painel.
              </Typography>
            </Alert>
          </Box>
